...
try {
  ...
  if ($source.Summary.detailCount -ne 1) { throw 'Source order must have exactly one detail row.' }
  ...
  if ((Get-DecimalValue ([pscustomobject]@{ value = $sourceDetail.DQty }) 'value') -ne 10 -or (Get-DecimalValue ([pscustomobject]@{ value = $sourceDetail.DMrpQty }) 'value') -ne 10) {
