-- ============================================================================
-- ASM-TOP-001 完整数据恢复脚本 (简化版)
-- 使用当前数据库 ID：co-dev (公司), a0000000-0000-0000-0000-000000000001 (用户)
-- 计量单位：PCS (件)
-- ============================================================================

-- 禁用触发器以避免自动创建关联记录时的冲突
ALTER TABLE item DISABLE TRIGGER trg_event_after_sync_item;
ALTER TABLE item DISABLE TRIGGER trg_event_async_ins_item;
...
-- 为所有物品创建 itemReplenishment 记录
INSERT INTO "itemReplenishment" ("itemId", "createdBy", "companyId", "createdAt")
SELECT id, "createdBy", "companyId", NOW() FROM item WHERE id LIKE 'item-asm%'
ON CONFLICT ("itemId") DO NOTHING;

-- 为所有物品创建 itemCost 记录
INSERT INTO "itemCost" ("itemId", "costingMethod", "createdBy", "companyId", "createdAt")
SELECT id, 'FIFO', "createdBy", "companyId", NOW() FROM item WHERE id LIKE 'item-asm%'
ON CONFLICT ("itemId") DO NOTHING;

-- 为所有物品创建 itemUnitSalePrice 记录
INSERT INTO "itemUnitSalePrice" ("itemId", "currencyCode", "createdBy", "companyId", "createdAt")
SELECT id, 'CNY', "createdBy", "companyId", NOW() FROM item WHERE id LIKE 'item-asm%'
ON CONFLICT ("itemId", "currencyCode") DO NOTHING;

-- 为所有物品创建 itemPlanning 记录 (针对现有位置)
INSERT INTO "itemPlanning" ("itemId", "locationId", "createdBy", "companyId", "createdAt")
SELECT i.id, l.id, i."createdBy", i."companyId", NOW()
FROM item i, location l
WHERE i.id LIKE 'item-asm%' AND l."companyId" = 'co-dev'
ON CONFLICT ("itemId", "locationId") DO NOTHING;

-- 为 Make 类型的物品创建 makeMethod 记录
INSERT INTO "makeMethod" (id, "itemId", "companyId", "createdBy", "createdAt", "defaultMethodType")
SELECT 'make-' || id, id, "companyId", "createdBy", NOW(), 'Make to Order'
FROM item
WHERE id LIKE 'item-asm%' AND "replenishmentSystem" = 'Make'
ON CONFLICT (id) DO NOTHING;

COMMIT;

-- 重新启用触发器
ALTER TABLE item ENABLE TRIGGER trg_event_after_sync_item;
ALTER TABLE item ENABLE TRIGGER trg_event_async_ins_item;

-- ============================================================================
-- 5. 创建工作中心 (Work Centers)
-- ============================================================================
