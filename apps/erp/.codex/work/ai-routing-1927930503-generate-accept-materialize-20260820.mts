...
const readableId = process.env.AI_ROUTING_READABLE_ID ?? "1927930503";
...
try {
  ...
  let tx;
  try {
    const draftId = await insertDraft(c, { companyId, userId, itemId: item.id, targetMakeMethodId: targetMakeMethod.id, draft, rationale: { runMarker, authorization: "User authorized controlled local generate-accept-materialize for 1927930503 on 2026-08-20", latestExtractionId: target.id, trainingSampleCount: samples.length } });
